...
const PlantAnalyticsModal = ({ isOpen, onClose, usina }) => {
    ...
    const getTariffValues = () => {
        ...
        // Mesma regra do PowerPlantModal do CRM e da pagina publica da usina.
        // GD1 nao paga Fio B.
        const isGD1 = uDetails.modalidade_gd === 'GD1';
        ...
        const isCompartilhada = uDetails.modalidade === 'geracao_compartilhada';
        ...
        if (isCompartilhada) {
            if (temAliquotas) {
                ...
                // Aliquota ausente nao vira zero: zero exibiria ao investidor
                // uma tarifa liquida ~34% maior que a real.
    ...
    const fetchAnalytics = async () => {
        ...
        try {
            ...
            const { data: usinaDetails } = await supabase.from('usinas')
                .select('valor_investido, ibge_code, potencia_kwp, supplier_id, unidade_geradora, concessionaria, grupo_tarifario, gestao_percentual, modalidade, modalidade_gd')
                                                ...
                                                {values.isCompartilhada && values.tributosPendentes && (
                                                    <div style={{ gridColumn: '1 / -1', background: '#fef2f2', padding: '0.9rem 1rem', borderRadius: '12px', border: '1px solid #fecaca' }}>
                                                        <span style={{ fontSize: '0.8rem', color: '#991b1b', fontWeight: 600 }}>
                                                            Esta usina é de Geração Compartilhada e sofre dedução de ICMS, PIS e COFINS, mas as alíquotas da concessionária ainda não foram cadastradas. A tarifa líquida abaixo está incompleta.
                                                        </span>
                                                    </div>
                                                )}
